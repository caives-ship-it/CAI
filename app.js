...
function goTab(tabId){
  ...
  if (["asistencia","actividades","sesiones","estadisticas","exportar","config"].includes(tabId)){
    renderTab(tabId);
  }
}
...
function setupNav(){
  ...
  document.getElementById("adminToggle").addEventListener("click", () => {
    if (isStaff()){
      ...
      toast("Modo staff desactivado", "ok");
      goTab("registro");
      updateAdminToggleLabel();
    } else {
      openStaffGate("asistencia");
    }
  });
  updateAdminToggleLabel();
}
...
function updateAdminToggleLabel(){
  document.getElementById("adminToggle").textContent = isStaff() ? "Salir de modo staff" : "Modo staff";
}
...
function renderAdvisoryStrip(){
  const wrap = document.getElementById("advisoryStrip");
  wrap.innerHTML = SEED_SERVICIOS_RECURRENTES.map(s => `
    <div class="advisory-chip"><strong>${esc(s.nombre)}</strong>${esc(s.horario)} · ${esc(s.detalle)}</div>
  `).join("");
}
...
function renderActividadesTab(){
  ...
  grid.innerHTML = STATE.actividades.map(a => `
    <div class="actividad-card ${a.activa===false?"inactiva":""}">
      ...
      <p class="meta">📍 ${esc(a.espacio)} · ⏱ ${esc(a.duracion)} · Cupo ${esc(String(a.cupo))}</p>
...
function actividadFormHtml(a){
  ...
  return `
    ...
    <div class="grid-2">
      <label>Tipo
        <select id="af_tipo">
          <option value="Taller" ${a.tipo==="Taller"?"selected":""}>Taller</option>
          <option value="Club de conversación" ${a.tipo==="Club de conversación"?"selected":""}>Club de conversación</option>
        </select>
      </label>
...
function renderSesionesTab(){
  const tbody = document.querySelector("#sesionesTable tbody");
  const rows = STATE.sesiones.slice().sort((a,b) => (b.fecha+b.hora).localeCompare(a.fecha+a.hora));
  tbody.innerHTML = rows.map(s => {
    ...
    return `
      <tr>
        <td>${esc(act ? act.nombre : "(eliminada)")}</td>
    ...
    `;
  }).join("") || `<tr><td colspan="8" class="muted-sm">No hay sesiones programadas.</td></tr>`;
}

function newSesionFormHtml(){
  const opts = STATE.actividades.filter(a => a.activa !== false)
...
function newSesion(){
  ...
  openModal(newSesionFormHtml());
  // prellenar cupo con el de la actividad seleccionada
  ...
  document.getElementById("sfSave").addEventListener("click", () => {
    const actividadId = actSel.value;
    ...
    if (!actividadId || !fecha) return toast("Selecciona actividad y fecha.", "error");
    STATE.sesiones.push({ id: uid("ses"), actividadId, fecha, hora, cupo, estado: "abierta" });
    persist();
    closeModal();
...
function renderTab(tabId){
  ...
  if (tabId === "sesiones") renderSesionesTab();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("newSesionBtn").addEventListener("click", newSesion);
  ...
  if (["asistencia","actividades","sesiones","estadisticas","exportar","config"].includes(initialTab) && !isStaff()){
    goTab("registro");
  } else {
